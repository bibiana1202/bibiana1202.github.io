...
컴퓨터공학을 전공했지만 졸업 후 간호대에 진학해 간호사로 일했습니다. 대학병원 병동에서 전자의무기록을 매일 쓰면서, **현장에서 무엇이 불편하고 어떤 설계가 일을 막는지**를 사용자 입장에서 겪었습니다.
...
### 💰 핀테크 서비스 백엔드 개발 ( 2025.01 ~ 현재 )

> **핀테크(FinTech)** 관련 웹·모바일 애플리케이션의 **백엔드 개발**과 **시스템 아키텍처 설계**를 담당합니다.
>
> - **핀테크 서비스 개발 · 운영**: 가입자 약 5천 명 규모 투자 플랫폼의 추천 조직 기반 수당·등급 산정과 월 정기 정산 스케줄러를 단독 구현, 운영 중 기능 개선부터 인도네시아 현지 버전 신규 구축·데이터 이관까지 담당
> - **멀티체인 지갑 서비스 개발· 운영**: EVM·Solana·TON·Tron·XRP·UTXO 계열을 지원하는 암호화폐 지갑 백엔드 설계 및 구현, 실시간 입금 감지·전송 상태 추적은 별도 프로세스로 분리
> - **Web3 리워드 서비스 개발 · 운영**: 가입자 약 3천 명 규모 리워드 애플리케이션에서 광고·미션 적립 포인트의 온체인 토큰 전환, 추천 조직 기반 수당, 리워드 지급 로직 개발 및 운영
> - **인프라 · 배포 · 운영**: 소규모 서버실에서 개발·메일 서버를 구축·운영, AWS에서는 트래픽 증가를 고려한 Auto Scaling·ALB·RDS 자동 확장 구조와 로그·백업 체계 설계, GitHub Actions self-hosted runner로 배포 자동화

### 🏥 병원정보시스템 개발 ( 2023.01 ~ 2024.09 )

> 간호사 면허를 보유한 개발자로 서울대학교병원·보라매병원·우리들병원의 **EMR/OCS**를 개발했습니다. 간호기록·환자교육·투석 등 **간호 업무 영역**을 주로 담당해 의료진 요구사항을 직접 해석하고 반영했습니다.
>
> - **간호 업무 영역 담당**: 간호기록·처치·환자교육 등 간호사가 사용하는 화면을 주로 개발·개선
> - **대학병원 · 전문병원 EMR**: 서울대학교병원·보라매병원·우리들병원의 온프레미스 및 웹 기반 EMR/OCS 개발·유지보수
> - **Oracle 임상 데이터**: 대용량 임상 데이터베이스 개발·운영
> - 자세한 업무 내용
>     - **우리들 병원 에서 진행했던 업무 (ON-PREMISE 기반 EMR)**
>         - Visual Basic 6 기반 application 개발 / 운영
>         - Database(Oracle) 대용량 데이터베이스 개발 / 운영
>     - **서울대학교병원에서 진행했던 업무(ON-PREMISE 기반 EMR)**
>         - Database(Oracle) 대용량 데이터베이스 개발 / 운영
>         - C# 기반 의료정보시스템 개발 및 유지보수
>         - 경구 수분섭취량 컨텐츠 수정 건
>         - HIS 메뉴 [교육실교육스케줄등록] 에서 교육명 중 code [1036, 질환소개] 항목의 삭제를 요청 건
>         - 스마트병실 시스템 분만장 관련 환자 중복 데이터 조건 반영 요청 건
>         - 중심정맥관관리 추가입력 건
>         - 퇴원간호기록 퇴원시 상태항목 수정 건
>         - 안과리뷰 화면 개발 건
>         - **[상처전담간호사의뢰] 의뢰부위입력 및 사진업로드 기능 추가, 의뢰 사유 수정 건**
>     - **보라매병원 에서 진행했던 업무 (WEB 기반 EMR)**
>         - Database(Oracle) 대용량 데이터베이스 개발 / 운영
>         - C# 기반 의료정보시스템 개발 및 유지보수
>         - [asp.net](http://asp.net) 기반 웹 의료정보 시스템 개발 및 유지보수
>         - 재활의학과에서 발생하는 L015503오더가 환자 진료후 안내문에 표기되기를 요청드리며 해당 검사실 장소는 1동 2층 이비인후과 언어치료실로 표기되도록 요청 건
>         - 진료 후 안내문에  혈액검사 표출 건
...
![DigMol 애플리케이션 화면](assets/portfolio/digmol-app-strip.jpg)

> 사용자가 광고 시청·미니게임으로 적립한 포인트를 BSC 상의 **MOL 토큰으로 전환·청구**하는 Web3 리워드 애플리케이션입니다. Flutter 앱·Node.js 백엔드·Solidity 스마트컨트랙트로 구성되며, 인증·포인트 도메인 등 서버 백엔드를 담당했습니다.
>
...
> - **운영 안정성 · 시각 정합성**: 백그라운드 워커를 API 서버 프로세스에서 분리해 워커 실행 ID로 실행 경로를 추적하고, 시각 값이 DB 서버 타임존에 좌우되던 문제를 전수 정리했습니다.
> - **온체인 조회 · 거래내역**: 외부 RPC 사용량이 서비스 한도를 넘길 위험을 점검해 조회 구조를 개선하고, 본드 예치·언스테이크·출금 이력을 인덱서로 확장해 거래내역을 제공합니다.
> - 아키텍처 구조
>
>     ![DigMol 아키텍처](assets/portfolio/digmol-architecture.png)
>
...
![Kookminwallet 애플리케이션 화면](assets/portfolio/kookminwallet-app-strip.jpg)
...
> - **AWS 인프라 구성**: ALB(ACM·HTTPS 종료) 뒤에 Auto Scaling Group을 두 가용영역에 분산 배치하고, 서버가 여러 대로 늘어나면 요청 제한 카운터가 인스턴스마다 따로 세어지는 문제를 ElastiCache(Valkey) 공유로 해결했습니다.
> - **네트워크 격리**: 애플리케이션 서버는 NAT를 통해서만 외부로 나가는 프라이빗 서브넷에, RDS는 인터넷 경로가 아예 없는 격리 서브넷에 배치했습니다.
...
> - 인프라 구조
>
>     ![Kookminwallet 인프라](assets/portfolio/kookminwallet-infra.png)
>
...
![SMBPIE 애플리케이션 화면](assets/portfolio/smbpie-app-strip.jpg)

> 가입자 약 5천 명 규모의 투자 플랫폼입니다. **현금 입금을 정해진 비율로 코인에 배분하는 입금 정산**, **보유 코인의 현금 출금 신청**, **추천 조직 기반 수당의 온체인 지급**을 담당했습니다. 국내 버전과 글로벌 버전을 병행 개발했습니다.
>
...
> - 아키텍처 구조
>
>     ![SMBPIE 아키텍처](assets/portfolio/smbpie-architecture.png)
>
...
![Coxign 관리자 화면](assets/portfolio/coxign-admin-strip.jpg)
...
> - 아키텍처 구조
>
>     ![Coxign 관리자 구조](assets/portfolio/coxign-admin-structure.png)
>
>     ![Coxign KYC 처리 흐름](assets/portfolio/coxign-kyc-flow.png)
>
...
![EMR 및 OCS 화면](assets/portfolio/emr-strip-final.jpg)
...
> 병리 슬라이드와 흉부 X-ray를 대상으로 의료 영상 딥러닝 경진대회 2건을 **팀장으로 총괄**했습니다.
>
